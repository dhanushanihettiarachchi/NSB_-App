// NSB_Booking/app/ManagerDashboard.tsx
// app/ManagerDashboard.tsx
...
export default function ManagerDashboard() {
  ...
  const role = String(params.role ?? 'Branch Manager');

  const fullName = useMemo(() => {
    const n = `${firstName} ${lastName}`.trim();
    return n.length ? n : 'Manager';
  }, [firstName, lastName]);
  ...
  const goToNotifications = () =>
    router.push({
      pathname: '/ManagerNotifications',
      params: { userId, firstName, lastName, email, role },
    });

  // Hover/press highlight states (only for icon background)
  const [hoveredIcon, setHoveredIcon] = useState<'bungalows' | 'bookings' | 'notifications' | null>(null);

  const isIconActive = (key: 'bungalows' | 'bookings' | 'notifications', pressed: boolean) =>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.mainWrap}>
            <View style={styles.card}>
              ...
              {/* ✅ Notifications (Manager extra) */}
              <Pressable
                style={styles.actionRow}
                onPress={goToNotifications}
                onHoverIn={() => setHoveredIcon('notifications')}
                onHoverOut={() => setHoveredIcon((p) => (p === 'notifications' ? null : p))}
              >
                {({ pressed }) => {
                  const active = isIconActive('notifications', pressed);
                  return (
                    <>
                      <View style={active ? styles.iconYellow : styles.iconOutline}>
                        <Ionicons
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.actionSub}>Approved bookings for your circuit</Text>
                      </View>
          ...
          <View style={styles.bottomBar}>
            ...
            {/* ✅ Bottom-right: Notifications (Profile removed) */}
            <TouchableOpacity style={styles.navBtn} onPress={goToNotifications} activeOpacity={0.9}>
              <View style={styles.navIconWrap}>
                <Ionicons name="notifications-outline" size={22} color={YELLOW} />
              </View>
                ...
                <View style={drawerStyles.profileHead}>
                  <View style={drawerStyles.avatar}>
                    <Text style={drawerStyles.avatarText}>
                      {(firstName?.[0] || fullName?.[0] || 'M').toUpperCase()}
                    </Text>
                  </View>
                  <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  navBtn: { width: 92, alignItems: 'center', justifyContent: 'center', gap: 6 },
  navBtnCenter: { width: 92, alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: -24 },
  ...
  navLabel: { fontSize: 11, fontWeight: '900', color: 'rgba(255,255,255,0.80)' },
});
