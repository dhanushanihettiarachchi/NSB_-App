...
// /payments/status-bulk?ids=1,2,3
  ...
  try {
    ...
    for (const row of result.recordset) {
      map[String(row.booking_id)] = {
        ...
        status: row.status,
        slipPath: row.payment_slip_path, // ✅ keep as relative path
      };
    }
...
// ✅ Get latest payment row for a booking (used by AdminReservations + UserBookings)
// Returns slip path as RELATIVE url: "/uploads/payments/xxx.png"
router.get('/booking/:bookingId/latest', async (req, res) => {
  try {
    ...
    return res.json({
      payment: {
        ...row,
        proof_url: row.payment_slip_path, // ✅ keep RELATIVE
      },
    });
  } catch (e) {
...
router.post('/upload-slip', upload.single('file'), async (req, res) => {
  try {
    ...
    return res.json({
      ...
      proof_url: slipPath, // ✅ keep RELATIVE
    });
  } catch (e) {
